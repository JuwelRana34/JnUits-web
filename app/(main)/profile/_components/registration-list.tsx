  ...
  return (
    <div className="grid gap-3">
      {data.map((reg) => (
        <div
          key={reg.id}
          className="group bg-card flex flex-col gap-2 rounded-xl border p-4 shadow-sm transition-all hover:border-indigo-500/30 hover:shadow-md sm:flex-row sm:items-center sm:justify-between sm:gap-4 dark:bg-zinc-950/50"
        >
          {/* Title + Date */}
          <div className="min-w-0 flex-1 space-y-1">
            <p className="line-clamp-2 font-semibold text-zinc-900 sm:truncate dark:text-zinc-100">
              {reg.event?.title ??
                reg.event?.type?.replace(/_/g, ' ') ??
                'Registration'}
            </p>
            <span className="text-muted-foreground text-xs font-medium tracking-wider uppercase">
              Applied{' '}
          ...
          {/* Badge — mobile তে left align, desktop তে right */}
