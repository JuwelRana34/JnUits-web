  ...
  const handleSignIn = (e: React.FormEvent) => {
    ...
    StartTransition(async () => {
      await authClient.signIn.email(formData, {
        ...
        onError: (ctx) => {
          const errorMsg = ctx.error.message || 'Invalid credentials'
          setError(errorMsg)

          // email verified না হলে resend বাটন দেখাও
          if (
    ...
    await authClient.sendVerificationEmail(
      ...
      },
      {
        ...
        onError: (ctx) => {
          ...
          setResendMessage({
            type: 'error',
            text: ctx.error.message || 'Failed to send verification email.',
          })
        },
      }
    )
    ...
    <div className="w-full">
      <form className="mt-8 space-y-5" onSubmit={handleSignIn}>
        ...
        {showResendVerification && (
          <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-500/20 dark:bg-yellow-500/10">
            <p className="mb-3 text-center text-sm text-yellow-700 dark:text-yellow-400">
              Your email is not verified. Click below to receive a new
              verification link.
            </p>
            <button
              type="button"
              ...
              className="flex w-full items-center justify-center rounded-lg bg-yellow-500 py-2 text-sm font-semibold text-white transition-all hover:bg-yellow-600 disabled:opacity-60"
            >
              {resendLoading ? (
        ...
        <Button
          type="submit"
          disabled={loading}
          className="flex w-full items-center justify-center rounded-lg py-3 font-semibold text-white shadow-sm transition-all disabled:opacity-60 dark:bg-zinc-600 dark:hover:bg-zinc-700"
        >
